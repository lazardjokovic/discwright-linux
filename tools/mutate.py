...
MUTATIONS = [
    ...
    ("window ignoring the form's greying", "window.py",
     "                    w.set_sensitive(state.enabled)", "                    w.set_sensitive(True)"),
    ("window left locked after a build", "window.py",
     "    def _build_done(self, iso: Path | None, error: str | None) -> bool:\n        self.form.building = False",
     "    def _build_done(self, iso: Path | None, error: str | None) -> bool:\n        pass"),
    ("extra content usable during a build", "window.py",
     'lambda: self._choose_extra(False), "add_extra"))', "lambda: self._choose_extra(False)))"),
    ("a failed build reported as built", "window.py",
    ...
    ("backslash not doubled for GKeyFile", "xdg.py",
     'value.replace("\\\\", "\\\\\\\\")', "value"),
    ("a folder of game files flattened onto the disc", "layout.py",
     'if entry.source != "Files" or entry.folder is None:', "if True:"),
    ("an entry with no installer given one anyway", "layout.py",
     "    if exe is None:\n        return \"\"", "    if exe is None:\n        pass"),
    ("the menu not told where a game's files are", "layout.py",
     '"folder": disc_entry_folder(entries, i),', '"folder": "",'),
    ("executables offered smallest first", "games.py",
     "exes.sort(key=lambda p: (-p.stat().st_size, p.name.casefold()))",
     "exes.sort(key=lambda p: (p.stat().st_size, p.name.casefold()))"),
    ("the list of executables not capped", "games.py",
     "return exes[:limit]", "return exes"),
    ("an empty folder taken as a game", "games.py",
     'info.msg = "This folder has no files in it."', 'info.ok = True'),
    ("a folder of files read only at its top level", "games.py",
     'return [p for p in folder.rglob("*") if p.is_file()]',
     'return [p for p in folder.iterdir() if p.is_file()]'),
    ("the downloads folder not spotted", "games.py",
     "hits = [p for p in folder.iterdir() if p.is_dir() and _setup_exes(p)]", "hits = []"),
    ("which kind of folder an entry is not written down", "project.py",
    ...
    ("a question asked about a folder with nothing in it", "form.py",
     "        if not files:\n            return None", "        if False:\n            return None"),
    ("cancel treated as the answer that adds the folder", "form.py",
     "        if choice <= 0 or choice > len(self.executables):",
     "        if choice < 0 or choice > len(self.executables):"),
    ("the question asked about a folder refused for another reason", "form.py",
     'if info.ok or not info.msg.startswith("No GOG"):', "if info.ok:"),
    ("an installer from outside the folder accepted", "cli.py",
